  ...
  return [
    {
      ...
      image: show.data[0].show.image ? show.data[0].show.image.medium : "https://tinyurl.com/tv-missing"
...
async function getEpisodesOfShow(id) { 
  ...
  let epArr = [];
  const eps = show.data.length;
  //Put the information in an easy to use array
  for(let ep = 0; ep < eps; ep++) {
    let epObj = {
      id: show.data[ep].id,
      name: show.data[ep].name,
      season: show.data[ep].season,
      number: show.data[ep].number
    }
    epArr.push(epObj);
  }
